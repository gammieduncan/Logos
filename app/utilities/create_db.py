...
if config.DB_TYPE == 'postgres':
    if config.USE_RECREATE == False:
        conn = psycopg2.connect(host=config.PG_HOST, database=config.DATABASE, user=config.PG_USER,
...
for i in range(50):
    title = fake.text()[:randint(10, 200)]
    new_post = Post(title=title, inurl_title=convert_ied(title),
                    self_text=pseudo_markup(fake.text(2000))[:randint(500, 2000)],
                    author='test', author_id=1, sub='test', ups=randint(1, 20), downs=randint(1, 5),
                    post_type='self_post', author_type='mod')
    db.session.add(new_post)
    db.session.commit()
    new_post.created = new_post.created - timedelta(days=randint(0, 8))
    new_post.permalink = config.URL + '/i/' + new_post.sub + '/' + str(new_post.id) + '/' + new_post.inurl_title + '/'
...
new_comment = Comment(post_id=1, sub_name='test', text='this is comment text', author='test', author_id=1,
                      ups=randint(1, 20), downs=randint(1, 5),
                      author_type='mod')
...
new_comment = Comment(post_id=1, sub_name='test', text='this is a reply', author='test', author_id=1, parent_id=1,
                      ups=randint(1, 20),
...
for i in range(200):
    ...
    new_comment = Comment(post_id=1, sub_name='test', text=pseudo_markup(fake.text()[:randint(1, 200)]), author='test',
                          author_id=1,
