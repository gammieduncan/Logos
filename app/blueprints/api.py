...
import config
from functions.db_functions import normalize_username, normalize_sub
from functions.functions import sqla_to_json, convert_ied
from ieddit import get_user_from_name
from models import Api_key, Post, Comment, Vote, Sub
...
def new_comment():
    ...
    new_comment.permalink = config.URL + '/i/' + new_post.sub + '/' + str(
        new_post.id) + '/' + new_post.inurl_title + '/' + str(new_comment.id) + '/'
    db.session.commit()
