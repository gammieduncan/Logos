...
import config
from functions.functions import rstring, time_ago
from ieddit import send_email, get_blocked, only_cache_get
from models import Post, Comment, Iuser, Password_reset, Pgp, Hidden
from share import db, captcha, limiter, cache
...
def password_reset(email=None):
    if request.method == 'POST':
        ...
        etext = 'Please visit this link to reset your password: <a href="%s/user/password_reset?reset=%s">LINK</a>' % (
        config.URL, key)
...
def user_update_preferences():
    ...
    if cur_password is None or cur_password == '':
...
def show_obj():
    ...
    if 'username' in session:
        hid = db.session.query(Hidden).filter_by(post_id=post_id, comment_id=comment_id, other_user=other_user,
                                                 anonymous=False).delete()
        db.session.commit()
        print(iid)
