...
from models import Mod_action, Post, Moderator, Comment, Ban, Iuser, Sub
...
def mod_addmod():
    ...
    if db.session.query(Moderator).filter(func.lower(Moderator.username) == func.lower(username),
                                          Moderator.sub == sub).first() != None:
